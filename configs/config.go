...
func ConfigInit(pwd string, files ...string) {
	...
	err := congor.Load(&CommonConfig, files...)
	if err != nil {
		slog.Error("Unable to set config", "err", err)
		os.Exit(1)
	}

	if CommonConfig.JWT_SECRET_BASE64 != "" && CommonConfig.JWT_SECRET == "" {
		b, err := base64.StdEncoding.DecodeString(CommonConfig.JWT_SECRET_BASE64)
		if err != nil {
			slog.Error("Unable to set jwt secret config from base 64", "err", err)
			os.Exit(1)
		}
		CommonConfig.JWT_SECRET = string(b)
	}
	...
	stripe.Key = CommonConfig.STRIPE_SECRET_KEY
}
